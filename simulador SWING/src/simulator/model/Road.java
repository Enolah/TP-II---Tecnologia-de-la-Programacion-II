...
public abstract class Road extends SimulatedObject {
	...
	Road(String id, Junction srcJunc, Junction destJunc, int maxSpeed, int contLimit, int lenght, Weather weather) {
		...
		if (contLimit < 0)
			throw new IllegalArgumentException("Invalid value for conLimit");
		if (lenght <= 0)
			throw new IllegalArgumentException("Invalid value for lenght");
		if (src == null || dest == null || wea == null)
			throw new IllegalArgumentException("Invalid value for src||dest||wea, cannot be NULL");
	...
	void enter(Vehicle v) {
		if (v.getLocation() == 0 && v.getSpeed() == 0) {
			...
			throw new IllegalArgumentException("Invalid value, can be zero");
	}
	...
	public class MiCompi implements Comparator<Vehicle> {
		...
		public int compare(Vehicle v0, Vehicle v1) {
			int ok = -1;
			if (v0.getLocation() < v1.getLocation())
				ok = 1;
			else if (v0.getLocation() == v1.getLocation())
				ok = 0;
			else if (v0.getLocation() > v1.getLocation())
				ok = -1;
			return ok;
